...
meses <- seq(
  from =as.yearmon({lubridate::year(Sys.Date()) - 6}),
...
sidra$infla <- get_sidra(
  ...
  mutate(meses =as.yearmon(meses, format ="%Y%m")) |>
  tidyr::pivot_wider(names_from =grupo, values_from =preco)
...
sidra$preco <- subset(sidra$preco, sidra$preco$meses %in% as.yearmon(meses)) |>
  tidyr::pivot_wider(names_from =grupo, values_from =preco)
